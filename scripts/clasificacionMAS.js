document.addEventListener('DOMContentLoaded', function() {
    const form = document.getElementById('clasificacinMASForm');
    const idInput = document.getElementById('id');
    const caravanaInput = document.getElementById('caravana');
    const sweepstakeCheckbox = document.getElementById('MASCheckbox');
    const topperCheckbox = document.getElementById('nMASCheckbox');
    ...
    form.addEventListener('submit', function(event) {
        ...
        if (!sweepstakeCheckbox.checked && !topperCheckbox.checked) {
            responseDiv.textContent = 'Debe seleccionar una opcion.';
            return;
        }
        ...
        const formData = {
            ...
            mas: MASCheckbox.checked ? 'MAS' : 'general',
    ...
    guardarTablaBtn.addEventListener('click', function() {
        const registros = [];
        registrosTotales.forEach(registro => {
            registros.push([registro.id, registro.caravana, registro.mas]);
        });

        const csvContent = "data:text/csv;charset=utf-8," 
            + ["ID,Caravana,MAS", ...registros.map(e => e.join(','))].join("\n");

        const ahora = new Date();
        const nombreArchivo = `clasificacionMAS_${ahora.getFullYear()}-${(ahora.getMonth()+1).toString().padStart(2, '0')}-${ahora.getDate().toString().padStart(2, '0')}_${ahora.getHours().toString().padStart(2, '0')}-${ahora.getMinutes().toString().padStart(2, '0')}-${ahora.getSeconds().toString().padStart(2, '0')}.csv`;
